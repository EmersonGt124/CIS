...
def alerta_ok(Windows_title, title, text): #mensaje con un solo boton aceptar
	hwnd = ctypes.windll.user32.FindWindowW(None, Windows_title)
	result = ctypes.windll.user32.MessageBoxW(hwnd, text, title, 64)
	return result

def alerta_error(Windows_title, title, text):#mensaje con 3 botones "anular 3, reintentar 4, omitir 5"
	hwnd = ctypes.windll.user32.FindWindowW(None, Windows_title)
